...
// Wenn Einheit im Cardiac Arrest, dann Skript verlassen
if (_unit getVariable ["ace_medical_inCardiacArrest", false]) exitWith {};
...
if ((_curDamage / _maxDamage) > 0.5 && {!(_unit getVariable ["cbrn_autoDamage", false])}) then {
    ...
    [{
        ...
        if (_newDamage >= 100) then {
            ...
            _unit setVariable ["cbrn_autodamage", false];
            _unit setVariable ["cbrn_stoppedAutoDamage", true];
        };
